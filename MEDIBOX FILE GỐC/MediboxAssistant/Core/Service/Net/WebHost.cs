        ...
        public void Start()
        {
            ...
            if (EnableHTTPS)
            {
                ...
                this.mAPIService = new WebServiceHost(typeof(WebAPI), new Uri[1]
              {
                ...
                var cert = store.Certificates.Find(X509FindType.FindBySubjectName, "medibot.vn", true)[0];
                store.Close();
                ...
                bindPortToCertificate.StartInfo.Arguments = string.Format("http add sslcert ipport=0.0.0.0:{0} certhash={1} appid={{{2}}}", Port, cert.Thumbprint, Guid.NewGuid());
                bindPortToCertificate.Start();
                bindPortToCertificate.WaitForExit();

                this.mAPIService.AddServiceEndpoint(typeof(IWebAPI), binding, "");
