...
namespace Medibox
{
    public partial class FormViewChecker : FormBase
    {
        private const String TAG = "FormViewChecker";
        //Private
        ...
        private void bwAsync_WorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            switch (type)
            {
                case ProcessingType.LoadData:
                    {
                        UtilityListView.ListViewRefresh(mListViewData, mListChecker);
                    }
                    break;
                default:
                    break;
        ...
        private void bwAsync_Worker(object sender, DoWorkEventArgs e)
        {
            ...
            switch (type)
            {
                case ProcessingType.LoadData:
                    {
                        mListChecker = CheckerPresenter.GetCheckers();
                    }
                    break;
                default:
                    break;
        ...
        {
            using (FormEditChecker form = new FormEditChecker(null, mListChecker))
            {
                if (form.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    DoRefresh();
